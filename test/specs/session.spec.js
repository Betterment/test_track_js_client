    ...
    context('with stubbed visitor and split registry', function() {
        ...
        describe('#logIn()', function() {
            it('updates the tt_visitor_id in the cookie', function(done) {
                ...
                this.session.logIn('bettermentdb_user_id', 444).then(function() {
                    expect(this.visitor.linkIdentifier).to.be.calledOnce;
                    expect(this.visitor.linkIdentifier).to.be.calledWithExactly('bettermentdb_user_id', 444);
        ...
        describe('#signUp()', function() {
            it('updates the tt_visitor_id in the cookie', function(done) {
                ...
                this.session.signUp('bettermentdb_user_id', 444).then(function() {
                    expect(this.visitor.linkIdentifier).to.be.calledOnce;
                    expect(this.visitor.linkIdentifier).to.be.calledWithExactly('bettermentdb_user_id', 444);
                    expect(cookieStub).to.be.calledOnce;
                    expect(cookieStub).to.be.calledWithExactly('tt_visitor_id', 'other_visitor_id', {
